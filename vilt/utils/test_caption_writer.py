...
class TestCocoWriter(unittest.TestCase):
    ...
    def test_load_captions(self):
        self.assertEqual(
            len(caption_writer.load_captions(self.root_path, "train")), 900
        )
        self.assertEqual(len(caption_writer.load_captions(self.root_path, "val")), 100)

    def test_get_image_paths(self):
        self.assertEqual(
            len(caption_writer.get_image_paths(self.root_path, "train")), 900
        )
        self.assertEqual(
            len(caption_writer.get_image_paths(self.root_path, "val")), 100
        )
    ...
    def test_create_work_batches_indices(self):
        ...
        train_batches = caption_writer.create_work_batches_indices(
            train_image_paths, 100
        )
        val_batches = caption_writer.create_work_batches_indices(val_image_paths, 100)
        self.assertEqual(len(train_batches), 9)
        ...
        self.assertEqual(val_image_paths[val_i:val_j], val_image_paths)
